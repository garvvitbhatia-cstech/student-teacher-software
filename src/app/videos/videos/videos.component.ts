...
export class VideosComponent implements OnInit,OnDestroy {
  ...
  getListFromServer(form:any){
    this.appService.postData('videos/list',form).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      if(r.permission){
        this.videos = r.data;
  ...
  updateStatus(classID:string,status:string){
    const data = {};
    this.appService.putData('video/status/update/'+classID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");

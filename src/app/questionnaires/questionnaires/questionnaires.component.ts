...
export class QuestionnairesComponent implements OnInit,OnDestroy {
  ...
  getListFromServer(form:any){
    this.appService.postData('questionnaires/list',form).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      if(r.permission){
        this.questions = r.users.data;
  ...
  updateStatus(subjectID:string,status:string){
    const data = {};
    this.appService.putData('question/status/update/'+subjectID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");

...
export class HomeLocationsComponent implements OnInit {
  ...
  getListFromServer(form:any){
    this.appService.postData('home/location/list',form).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
  ...
  updateStatus(classID:string,status:string){
    const data = {};
    this.appService.putData('home/location/status/update/'+classID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");

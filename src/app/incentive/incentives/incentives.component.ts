...
export class IncentivesComponent implements OnInit,OnDestroy {
  ...
  getListFromServer(form:any){
    this.appService.postData('incentive/list',form).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      if(r.permission){
  ...
  updateStatus(classID:string,status:string){
    const data = {};
    this.appService.putData('incentive/status/update/'+classID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");

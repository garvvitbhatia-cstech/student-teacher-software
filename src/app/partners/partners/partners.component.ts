...
export class PartnersComponent implements OnInit,OnDestroy {
  ...
  getListFromServer(form:any){
    this.appService.postData('partners/list',form).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      if(r.permission){
  ...
  updateStatus(userID:string,status:string){
    const data = {};
    this.appService.putData('partner/status/update/'+userID+'/'+status,data).pipe(takeUntil(this.destroy$)).subscribe(res=>{
      var r:any=res;
      this.getList();
    },error=>{
      this.toastr.error("Server Error","Error");
